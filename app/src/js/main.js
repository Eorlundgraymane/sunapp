  ...
  xhr.onreadystatechange = function(){
    ...
    if(xhr.readyState == 4 && xhr.status == 200){
      ...
      for(each of res){
        ...
        console.log(id);
        if(author_id == id){
          document.getElementById('posts').innerHTML += '<li id = "post"><figure id = "auth_info"><img id = "auth_img" class = "img img-rounded img-responsive" src = "'+author_img+'"alt = "Author Image"><figcaption id = "auth_name">'+author_name+'</figcaption><button id = "deletepost" type = "button" class = "btn" onclick = "alert(\'Delete Posts Coming Soon...\');">X</button></figure><h1 id = "post_title">'+title+'</h1><figure><img id = "post_image" class = "img img-rounded img-responsive" src = "'+postimg+'" alt = "Post Image"><figcaption id = "post_text">'+post+'</figcaption></figure></li>';
        }
        else{
